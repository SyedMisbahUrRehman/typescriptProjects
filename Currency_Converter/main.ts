...
async function getExchangeRates(): Promise<{ [currency: string]: number } | null> {
  try {
    ...
    return (await response.json()) as { [currency: string]: number };
  } catch (error) {
    console.error(`Error fetching exchange rates: ${(error as Error).message}`);
    return null;
  }
}

async function convertCurrency(exchangeRates: { [currency: string]: number }) {
  const questions = [
    {
      type: 'input',
      name: 'amount',
